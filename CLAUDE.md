...
- Planner search: two bundled, committed catalogs — `public/catalogs/dso.json`
  (OpenNGC) and `public/catalogs/stars.json` (HYG v4.1: every star with a
  HIP/HD/HR number, ~118k rows, ~6 MB), both CC BY-SA 4.0 and regenerated by
  `scripts/generate-*-catalog.mjs` (network only at generation time).
  `src/lib/catalogSearch.ts` and `src/lib/starSearch.ts` are pure and
  unit-tested; `objectSearch.ts` merges them into one ranked list;
  `plannerTargets.ts` is the single place that builds a `PlannerTarget` from
  a hit or from typed RA/Dec (`coords.ts`). Target ids are designations
  (`NGC 7000`, `HD 172167`, else `HIP n`) or `custom-<ms>` for coordinates;
  names are user-editable in the Planner list (`updatePlannerTarget`).
