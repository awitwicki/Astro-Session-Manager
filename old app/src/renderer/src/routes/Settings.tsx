...
export function Settings() {
  ...
  const loadCacheSize = async (): Promise<void> => {
    const size = await window.electronAPI.thumbnail.getCacheSize()
    setCacheSize(size)
  }
  ...
  const clearCache = async (): Promise<void> => {
    setClearing(true)
    try {
      await window.electronAPI.thumbnail.clearCache()
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      ...
      <div className="settings-group">
        ...
        <p className="settings-description">
          Maximum temperature difference (in °C) when matching darks to lights. Default: 2°C.
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <input
            type="range"
