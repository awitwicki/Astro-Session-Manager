...
export function SessionView() {
  ...
  const generateThumbnails = useCallback(async () => {
    ...
    const unsubscribe = window.electronAPI.thumbnail.onProgress((_event, data) => {
      setProgress({ current: data.current, total: data.total })
    })

    try {
      const results = await window.electronAPI.thumbnail.batchGenerate(missingPaths)
  ...
  useEffect(() => {
    ...
    window.electronAPI.fits.batchReadHeaders(paths).then((headers) => {
  ...
  const handleDelete = async (): Promise<void> => {
    for (const filePath of selectedFrames) {
      const result = await window.electronAPI.file.moveToTrash(filePath)
      if (result.success) {
