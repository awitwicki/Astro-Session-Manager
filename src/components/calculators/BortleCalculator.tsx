...
function ResultCard({ label, value, unit }: { label: string; value: number | null; unit: TimeUnit }) {
  return (
...
export function BortleCalculator() {
  ...
  const factorBroadband = isCustomBortle ? parseNum(customBroadband) : selectedRow?.broadband ?? null
  const factorHa = isCustomBortle ? parseNum(customHa) : selectedRow?.ha ?? null
  const factorOiii = isCustomBortle ? parseNum(customOiii) : selectedRow?.oiii ?? null
  ...
  return (
    <section className="calc-section">
      ...
      <p className="calc-section-desc">
        Relative integration time needed to gather the same signal as under dark
        Bortle&nbsp;2 (base) skies. Broadband suffers most from light pollution;
        narrowband (Ha&nbsp;3&nbsp;nm, O&nbsp;III&nbsp;3&nbsp;nm) is far more resilient.
      </p>
        ...
        <table className="calc-table">
          ...
          <tbody>
            {BORTLE_ROWS.map((row) => {
              ...
              return (
                <tr key={row.bortle} className={isActive ? 'calc-row-active' : undefined}>
                  <td>
                    {row.bortle}
      ...
      <div className="calc-panel">
        <div className="calc-fields">
          <div className="calc-field">
            <label className="calc-field-label" htmlFor="bortle-base-time">Base time (at Bortle 2)</label>
          ...
          <div className="calc-field">
            <label className="calc-field-label" htmlFor="bortle-select">Your Bortle</label>
            <select
              id="bortle-select"
              className="calc-select"
              value={bortleSel}
            ...
            >
              {BORTLE_ROWS.map((row) => (
                <option key={row.bortle} value={String(row.bortle)}>
                  Bortle {row.bortle}{row.bortle === BORTLE_BASE ? ' (base)' : ''}
                </option>
              ))}
          ...
          {isCustomBortle && (
            <>
              <div className="calc-field">
                <label className="calc-field-label" htmlFor="custom-broadband">Broadband (×)</label>
              ...
              <div className="calc-field">
                <label className="calc-field-label" htmlFor="custom-ha">Ha 3 nm (×)</label>
              ...
              <div className="calc-field">
                <label className="calc-field-label" htmlFor="custom-oiii">O III 3 nm (×)</label>
        ...
        <div className="calc-results">
          <ResultCard label="Broadband" value={requiredTime(factorBroadband)} unit={bortleUnit} />
          <ResultCard label="Ha/SII 3 nm" value={requiredTime(factorHa)} unit={bortleUnit} />
          <ResultCard label="O III 3 nm" value={requiredTime(factorOiii)} unit={bortleUnit} />
        </div>
      </div>
    </section>
  )
