...
export function ClassicSkyView() {
  ...
  useEffect(() => {
    ...
    ensureCelestialLoaded().then(() => {
      ...
      // d3-celestial caches its internal SVG-ish data container on the
      // Celestial singleton and only recreates it when falsy. Since this is
      // an SPA where the container div is torn down and rebuilt by React on
      // every navigation (e.g. re-visiting Sky Map, or Sky Map after the
      // Planner detail chart has run display() once), a stale reference here
      // means display() re-appends stars/DSOs/graticule into a detached node
      // instead of the live one. Force a fresh container so display()
      // re-attaches to the current DOM.
      ...
      // Custom layers registered by other pages (or earlier mounts of this
      // one) persist inside d3-celestial across display() calls — drop them
      // so only this page's overlay draws here.
      Celestial.clear()
      ...
      recenterIntervalRef.current = reCenterInterval
    })
